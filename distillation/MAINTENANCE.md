...
3. **Choose the smallest useful change.**
   - Update `corpus/data/tweets.json` for every new tweet.
   - Update `track-record.md` for dated calls, validations, reversals, and
     calibration events.
   - Update `corpus/references/theses/` sector files (or root `theses.md` then re-run `scripts/split_theses.py`) for ticker-specific stance, evidence, risk, and latest view changes.
...
5. **Keep provenance compact.** Mention the date and the concrete signal, not a
   long quote. Preserve full text in `corpus/data/tweets.json`.
...
4. Classify each new post:
   ...
   - `article summary`
   - `skip skill update`
5. Make the smallest reference edit that captures the durable change — or run `python scripts/agent_distill.py --since-sync` for automated apply.
